...
//        mPopupWindow.setHeight((Hight - view.getBottom()) * 2 / 3);
    ...
    public PopWindowUtil makePopupWindow(Context context, View popuView) {
        DisplayMetrics dm = new DisplayMetrics();
        WindowManager wmManager=(WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        wmManager.getDefaultDisplay().getMetrics(dm);
        mPopupWindow = new PopupWindow(context);
        popuView.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
     ...
     *
     * @param cx 此处必须为Activity的实例
     * @param parent 显示在该控件之下
     * @param weight 在布局的位置
     * @param anim 弹出及消失动画
     * @return
     */
    public PopupWindow showLocationWithAnimation(final Context cx, View parent, int weight, int anim) {
        // 弹出动画
        mPopupWindow.setAnimationStyle(anim);
        // 弹出PopupWindow时让后面的界面变暗
        WindowManager.LayoutParams parms = ((Activity) cx).getWindow().getAttributes();
        parms.alpha =0.8f;
        ((Activity) cx).getWindow().addFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);
        ((Activity) cx).getWindow().setAttributes(parms);


        mPopupWindow.showAtLocation(parent,weight,0,0);
        mPopupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
            @Override
            public void onDismiss() {
                // PopupWindow消失后让后面的界面变亮
