...
public class FSKDecoder {
	...
	public interface FSKDecoderCallback {
		/**This method will be called every time there is new data received * @param newData */
	...
	/** Create FSK Decoder instance to feed with audio data* @param config* @param callback */
	...
	/**	 * Use this method to feed 8bit PCM data to the decoder	 * @param data
	 * @return samples space left in the buffer	 */
	public int appendSignal(byte[] data) {
		return appendSignal(byteArrayToShortArray(data));
	}

	/**	 * Use this method to feed 16bit PCM data to the decoder	 * @param data
	 *  * @return samples space left in the buffer	 */
		...
		synchronized (mSignal) {
			...
			if (mSignalEnd + monoData.length > mSignal.capacity()) {
				...
				if ((mSignalEnd + monoData.length)-mSignalPointer <= mSignal.capacity()) {
					...
					// refuse data and tell the amount of overflow
					return (mSignal.capacity() - (mSignalEnd + monoData.length));
				}
			}
			...
			return (mSignal.capacity() - mSignalEnd);
		}
	}

	/**	 * Use this method to set 8bit PCM data to the decoder	 * @param data
	 * @return samples space left in the buffer	 */
	...
	/**
	 * Use this method to set 16bit PCM data to the decoder 	 * @param data
	 * @return samples space left in the buffer	 */
	...
	protected int calcFrequencyZerocrossing(short[] data) {
		...
		double numSecondsRecorded = (double) numSamples	/ (double) mConfig.sampleRate;
		double numCycles = numCrossing / 2;
		double frequency = numCycles / numSecondsRecorded;

		vFreq= (int) Math.round(frequency);
		return (int) vFreq;//Math.round(frequency);
	}
	...
	protected void flushData() {
		if (mDataLength > 0) {
			...
			for (int i = 0; i < mDataLength; i++) {	data[i] = mData.get(i);	}
			allocateBufferData();
			mDataLength = 0;
			notifyCallback(data);
		}
	...
	protected void processIterationSearch() {
		if (mSignalPointer <= mSignalEnd-mConfig.samplesPerBit) {
			...
			if (mDecoderStatus.equals(DecoderStatus.SEARCHING_START_BIT) &&
					freq == mConfig.modemFreqLow && state.equals(STATE.LOW)) {
				//found start bit
	...
	protected void processIterationDecode() {
		...
		if (mSignalPointer <= mSignalEnd-mConfig.samplesPerBit) {
			...
			if (mCurrentBit == 0 && state.equals(STATE.LOW)) {
				//start bit 	//prepare buffers
				mBitBuffer = new StringBuffer();
				mCurrentBit++;
			}
			else if (mCurrentBit == 0 && state.equals(STATE.HIGH)) {
				//post-carrier bit(s) //go searching for a new transmission
				setStatus(DecoderStatus.SEARCHING_START_BIT);
			}
			...
			else if (mCurrentBit > 0 && mCurrentBit < 9 &&
					(state.equals(STATE.HIGH) || state.equals(STATE.LOW))
					) {
			...
			trimSignal(); //get rid of data that is already processed
			flushData();
			//we need to wait for more data to continue decoding
