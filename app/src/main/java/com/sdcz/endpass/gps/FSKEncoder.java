...
public class FSKEncoder {
	...
	protected enum STATE {		HIGH, LOW, SILENCE	}
	protected enum EncoderStatus {		IDLE, PRE_CARRIER, ENCODING, POST_CARRIER, SILENCE	}
	...
	/** Create FSK Encoder instance to feed with byte array data @param config  @param callback */
	public FSKEncoder(FSKConfig config, FSKEncoderCallback callback) {
		mConfig = config;
		mCallback = callback;
		//(int) Math.ceil(mConfig.sampleRate * 40.0f / 1000.0f / mConfig.samplesPerBit);
		mPreCarrierBits = FSKConfig.ENCODER_PRE_CARRIER_BITS;
		//(int) Math.ceil(mConfig.sampleRate * 5.0f / 1000.0f / mConfig.samplesPerBit);
		mPostCarrierBits = FSKConfig.ENCODER_POST_CARRIER_BITS;
		//(int) Math.ceil(mConfig.sampleRate * 5.0f / 1000.0f / mConfig.samplesPerBit);
		mSilenceBits = FSKConfig.ENCODER_SILENCE_BITS;
	...
	protected void notifyCallback(byte[] pcm8, short[] pcm16) {
		if (mCallback != null) { mCallback.encoded(pcm8, pcm16);}
	}
	 ...
	 * @param data* @return bytes left in the buffer */
		...
		synchronized (mData) {
			if (mDataLength + data.length > mData.capacity()) {
				...
				if ((mDataLength + data.length)-mDataPointer <= mData.capacity()) {
					...
					//the encoder is gagging // refuse data and tell the amount of overflow
	...
	/**Use this method to set byte array data to the encoder @param data
	...
	protected void checkSignalBuffer() {
		if (mConfig.pcmFormat == FSKConfig.PCM_8BIT) {
			if (mSignalLength >= mSignalPCM8.capacity() - 2) {//time to flush
				flushSignal();
			}
		}
		else {
			if (mSignalLength >= mSignalPCM16.capacity() - 2) {	//time to flush
				flushSignal();
			}
		}
	}
	...
	protected byte[] modulate8(STATE state) {
		...
		for (int i = 0; i < buffer.length; i++) {
			buffer[i] = (byte) (128 + 127 * Math.sin((2 * Math.PI)
					* (i*1.0f / mConfig.sampleRate) * freq));
	    }
	...
	protected short[] modulate16(STATE state) {
		...
		for (int i = 0; i < buffer.length; i++) {
			buffer[i] = (short) (32767 * Math.sin((2 * Math.PI) *
					(i*1.0f / mConfig.sampleRate) * freq));
	    }
